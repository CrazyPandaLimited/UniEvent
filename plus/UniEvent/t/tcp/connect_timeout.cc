...
//TEST_CASE("connect to nowhere", "[tcp-connect-timeout]") {
...
//}
//
//TEST_CASE("connect timeout with real connection", "[tcp-connect-timeout]") {
...
//
//    client->connect().to("localhost", panda::to_string(port)).timeout(20);
//
//    client->connect_event.add([&](Stream*, const CodeError* err, ConnectRequest*) {
//        CHECK_FALSE(err);
//    });
...
TEST_CASE("connect timeout with real canceled connection", "[tcp-connect-timeout]") {
    if (variation.socks) return;
    int connected = 0;
    int errors = 0;
    int successes = 0;
    int tries = variation.ssl ? 100 : 5000;
    auto port = find_free_port();
...
//
////TEST_CASE("connect timeout with error connection", "[tcp-connect-timeout]") {
//    //AsyncTest test(250, {});
//
...
//
//
//TEST_CASE("connect timeout with black hole", "[tcp-connect-timeout]") {
...
//}
//
//TEST_CASE("connect timeout clean queue", "[tcp-connect-timeout]") {
...
//}
//
//TEST_CASE("connect timeout with black hole in roll", "[tcp-connect-timeout]") {
...
//}
//
//TEST_CASE("regression on not cancelled timer in second (sync) connect", "[tcp-connect-timeout]") {
//    auto port = find_free_port();
//    bool cached_resolver;
//    SECTION("ordinary resolver") {
//        cached_resolver = false;
