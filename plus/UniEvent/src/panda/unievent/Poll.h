...
public:
    using poll_fptr = void(Poll* handle, int events, const PollError& err);
    ...
    void call_on_poll (int events, const PollError& err) { on_poll(events, err); }

protected:
    virtual void on_poll (int events, const PollError& err);
