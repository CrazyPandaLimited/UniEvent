...
void StreamFilter::on_connect  (const StreamError& err, ConnectRequest* req) { next_on_connect(err, req); }
void StreamFilter::on_write    (const StreamError& err, WriteRequest* req)   { next_on_write(err, req); }
void StreamFilter::on_read     (const string& buf, const StreamError& err)   { next_on_read(buf, err); }
void StreamFilter::on_shutdown (const StreamError&, ShutdownRequest*)        {}
