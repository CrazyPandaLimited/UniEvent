...
string  CodeError::name  () const { return _name; }
string  CodeError::descr () const { return _descr; }

string CodeError::_mkwhat () const {
    if (!_code) return {};
    return name() + '(' + string::from_number(_code.value()) + ") " + descr();
}

CodeError* CodeError::clone () const { return new CodeError(_code, _name, _descr); }
